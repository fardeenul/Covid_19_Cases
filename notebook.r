...
# Install packages into the custom library
install.packages("readr", lib = my_lib)
install.packages("ggplot2", lib = my_lib)
install.packages("dplyr", lib = my_lib)

# Load the packages
library(readr, lib.loc = my_lib)
library(ggplot2, lib.loc = my_lib)
...
plt_cum_confirmed_cases_china_vs_world <- ggplot(confirmed_cases_china_vs_world, aes(x = date, y = cum_cases, color = is_china)) +
  geom_line() +
  labs(y = "Cumulative Cases")
...
plt_cum_confirmed_cases_china_vs_world +
  geom_vline(data = who_events, aes(xintercept = date), linetype = "dashed", color = "red") +
  annotate("text", x = who_events$date, y = Inf, label = who_events$event, vjust = -0.5, color = "red")
...
plot(china_after_feb15$date, china_after_feb15$cum_cases, 
     xlab = "date", ylab = "cum_cases", main = "Line Plot")
...
ggplot(china_after_feb15, aes(x = date, y = cum_cases)) +
  ...
  ggtitle("Line Plot with Trend Line")
...
plot(not_china$date, not_china$cum_cases,xlab = "date", ylab = "cum_cases", main = "Line Plot") 
# Add a smooth trend line using linear regression, no error bars
...
plot(confirmed_cases_top7_outside_china$date, confirmed_cases_top7_outside_china$cum_cases) + 
 geom_line(color = "blue", linetype = "solid") +
 xlab("Date") +
          ylab("Cumulative Cases") 
